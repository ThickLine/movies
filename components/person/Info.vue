...
<template>
    <div p4
         grid="~ md:cols-[max-content_1fr]"
         gap-8
         items-center
         ma
         max-w-300>
      <NuxtImg v-if="props.item.profile_path"
               width="400"
               height="600"
               format="webp"
               :src="`${TMDB_IMAGE_BASE_THUMBNAIL}/${props.item.profile_path}`"
               :alt="props.item.name"
               block
               border="4 gray4/10"
               w-70
      ...
      <div p4
        ...
        <div text-sm
             op80>
          <ul grid="~ cols-[max-content_1fr] gap3"
              items-center>
        ...
        <div>
          <ExternalLinks :links="externalIds"  routeName="person"/>
      </div>
    </div>
  </div>
</template>
