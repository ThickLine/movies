...
<template>
  <div :key="item?.id"
       relative
       class="aspect-ratio-1/1 md:aspect-ratio-3/2 lg:aspect-ratio-25/9"
       bg-black>
    <div absolute
         top-0
         right-0
      ...
      <NuxtImg width="400"
               height="225"
               format="webp"
               :src="`${TMDB_IMAGE_BASE_THUMBNAIL}/${props.item.backdrop_path}`"
      ...
      <Transition appear
        ...
        <div v-show="mounted">
          ...
          <div flex="~ row wrap"
            ...
            <div v-if="props.item.release_date"
                 op50>
              {{ props.item.release_date.slice(0, 4) }}
            </div>
            <div v-if="props.item.runtime"
                 op50>
